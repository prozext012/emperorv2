...
Kelola produk, testimoni, status, chat pelanggan, dan pantau statistik kunjungan —
semua tersinkron langsung ke web utama tanpa perlu deploy ulang.
...
Repo ini adalah **dashboard admin dixzSTORE** — akses privat untuk mengelola seluruh
konten yang tampil di web utama ([`kaisarv1`](#-repo-terkait)). Dilindungi login
Firebase Authentication, hanya satu akun admin yang diizinkan mengakses.

Setiap perubahan di sini (produk baru, testimoni, status, dll) langsung muncul di web
utama secara real-time berkat `onSnapshot` Firestore.
...
- 🗂️ **Manajemen produk** — tambah, edit, hapus produk beserta gambar
- 📝 **Kelola testimoni & notifikasi** — atur konten yang tampil di web utama
- 📷 **Kelola status/story** — upload gambar/video, auto-expire
...
- 🧹 **Auto-cleanup terjadwal** — bersihkan data lama (status, chat, log) otomatis
...
1. Di **Firebase Console → Authentication → Sign-in method**, aktifkan provider
   **Email/Password**
...
4. Di **Google Cloud Console → Credentials**, pastikan API key dibatasi
   (*HTTP referrer restriction*) hanya untuk domain resmi
...
> ⚠️ Jangan pernah menaruh **password** di dalam kode — hanya alamat **email** admin
> yang boleh ditulis di source code (untuk validasi tampilan), karena email bukan data
> rahasia. Password admin **hanya** tersimpan di sistem Firebase Authentication.
